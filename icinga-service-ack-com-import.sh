...
INFILE="${INFILE:-services_acks_comments.json}"
...
jq -c '.ack_comments[]' "${INFILE}" | while read -r item; do
  ...
  FILTER="host.name==\"${HOST}\" && service.name==\"${SVC}\""
